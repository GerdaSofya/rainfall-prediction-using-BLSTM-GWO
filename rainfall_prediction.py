...
def predict_rainfall(data_input):
    ...
    for i in range(len(data_input)):
        normalize = (data_input[i] - min_value[i]) / (max_value[i] - min_value[i])
    ...
    model = load_model('model.h5', custom_objects={'rmsle': rmsle, 'rmse': rmse})
...
def main():
    ...
    # Memeriksa apakah button ditekan
    if clicked:
        ddd_car_code = {0: 'C', 1: 'E', 2: 'N', 3: 'NE', 4: 'NW', 5: 'S', 6: 'SE', 7: 'SW', 8: 'W'}
        ddd_car_value = list(ddd_car_code.keys())[list(ddd_car_code.values()).index(ddd_car)]

        data_input = [RR, Tn, Tx, ss, ff_x, ff_avg, ddd_x, float(ddd_car_value)]

        result = predict_rainfall(data_input)

        series = {
            "Hari ke-": [i+1 for i in range(len(result))],
            "rainfall": result
        }
